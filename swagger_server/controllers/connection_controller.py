...
def place_connection(body):  # noqa: E501
    ...
    # TODO: raise a more useful/expressive error.
    if topo_val is None:
        raise Exception("Could not find a topology in the DB")
    ...
    # TODO: runMC_Solver() errors out when we attempt to invoke it
    # outside of its native habitat; see
    # https://github.com/atlanticwave-sdx/pce/issues/49
    try:
        result = runMC_Solver()
    except Exception as e:
        print(f"Using work-around for runMC_Solver() error: {e}")
        # The below value is pre-computed; ie, generated by
        # runMC_Solver() when it is called "correctly", when all its
        # expected input files are in place. This is temporary (famous
        # last words!), and should go away when we have a more robust
        # implementation of runMC_Solver().
        result = [
            {
                '1': [[1, 9], [9, 11]],
